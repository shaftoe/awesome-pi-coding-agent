...
This is a Typescript project (Node.js + pnpm, tests via Vitest) that automatically discovers, enriches, and renders a curated list of resources for the [Pi Coding Agent](https://pi.dev/) ecosystem into an awesome-list README.

Currently under full rewrite, refer to docs/ARCHITECTURE.md and docs/REWRITE-PLAN.md for up to date state.

# Guidelines for coding agents

- **ALWAYS** run `pnpm run check` before considering a change complete and ready
- **ALWAYS** run `pnpm run format` to auto-fix lint/format issues (import ordering, etc.) before falling back to manual edits
